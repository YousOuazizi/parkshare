...
export class MLPricingAlgorithm implements PricingAlgorithm {
  ...
  async calculatePrice(
    ...
    try {
      ...
      const occupancyFactor = this.calculateOccupancyFactor(contextData?.occupancyRate);
      const demandFactor = this.calculateDemandFactor(contextData?.demandScore);
      const competitionFactor = this.calculateCompetitionFactor(contextData?.nearbyAvailability);
      ...
      this.logger.error(`Error in ML pricing algorithm: ${error.message}`, error.stack);
      
      // En cas d'erreur, retomber sur l'algorithme de base
      return this.basePricingAlgorithm.calculatePrice(basePrice, startTime, endTime);
    }
  }
  ...
  private calculateCompetitionFactor(nearbyAvailability: number): number {
    if (nearbyAvailability === undefined || nearbyAvailability === null) return 1.0;
