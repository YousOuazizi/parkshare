...
import { SubscriptionSharing, SharingStatus } from './entities/subscription-sharing.entity';
...
export class SubscriptionsService {
  ...
  async create(userId: string, createSubscriptionDto: CreateSubscriptionDto): Promise<Subscription> {
    ...
    const parking = await this.parkingsService.findOne(createSubscriptionDto.parkingId);
    ...
    if (!plan) {
      throw new NotFoundException(`Plan d'abonnement avec l'id ${createSubscriptionDto.planId} non trouvé ou inactif`);
    }
    ...
    if (existingSubscription) {
      throw new ConflictException('Vous avez déjà un abonnement actif pour ce parking');
    }
    ...
    switch (plan.type) {
      ...
      case 'daily':
        ...
        break;
      case 'weekly':
        endDate = new Date(startDate.getTime() + plan.duration * 7 * 24 * 60 * 60 * 1000);
        break;
      case 'monthly':
    ...
    // Calculer le prix
    const pricePerPeriod = this.calculateSubscriptionPrice(parking.basePrice, plan);
  ...
  async findAll(userId?: string, parkingId?: string, status?: SubscriptionStatus): Promise<Subscription[]> {
  ...
  async update(id: string, userId: string, updateSubscriptionDto: UpdateSubscriptionDto, isAdmin = false): Promise<Subscription> {
    const subscription = await this.findOne(id);
    
    // Vérifier les permissions
    if (subscription.userId !== userId && !isAdmin) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à modifier cet abonnement');
    }
  ...
  async cancel(id: string, userId: string, isAdmin = false): Promise<Subscription> {
    const subscription = await this.findOne(id);
    
    // Vérifier les permissions
    if (subscription.userId !== userId && !isAdmin) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à annuler cet abonnement');
    }
  ...
  async pauseSubscription(id: string, userId: string, pauseDto: PauseSubscriptionDto, isAdmin = false): Promise<Subscription> {
    const subscription = await this.findOne(id);
    
    // Vérifier les permissions
    if (subscription.userId !== userId && !isAdmin) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à mettre en pause cet abonnement');
    }
    
    // Vérifier s'il reste des pauses disponibles
    if (subscription.pausesRemaining <= 0) {
      throw new BadRequestException('Vous avez déjà utilisé toutes vos pauses disponibles');
    }
    ...
    if (pauseDuration > 30) {
      throw new BadRequestException('La durée de pause ne peut pas dépasser 30 jours');
    }
  ...
  async resumeSubscription(id: string, userId: string, isAdmin = false): Promise<Subscription> {
    const subscription = await this.findOne(id);
    
    // Vérifier les permissions
    if (subscription.userId !== userId && !isAdmin) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à reprendre cet abonnement');
    }
    ...
    if (activePause) {
      ...
      const remainingPause = activePause.endDate.getTime() - activeDate.getTime();
      
      // Ajuster la date de fin de l'abonnement pour enlever la durée restante de la pause
      subscription.endDate = new Date(subscription.endDate.getTime() - remainingPause);
  ...
  async shareSubscription(id: string, userId: string, shareDto: ShareSubscriptionDto): Promise<SubscriptionSharing> {
    const subscription = await this.findOne(id);
    ...
    if (subscription.userId !== userId) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à partager cet abonnement');
    }
    
    // Vérifier si l'abonnement est actif
    if (subscription.status !== SubscriptionStatus.ACTIVE) {
      throw new BadRequestException('Vous ne pouvez partager qu\'un abonnement actif');
    }
    ...
    if (!targetUser) {
      throw new NotFoundException(`Aucun utilisateur trouvé avec l'email ${shareDto.email}`);
    }
    ...
    if (existingSharing) {
      throw new ConflictException('Cet abonnement est déjà partagé avec cet utilisateur');
    }
    ...
    const sharing = this.sharingRepository.create({
      ...
      validUntil: shareDto.validUntil ? new Date(shareDto.validUntil) : subscription.endDate
    });
  ...
  async respondToSharing(sharingId: string, userId: string, accept: boolean): Promise<SubscriptionSharing> {
    ...
    if (sharing.sharedWithUserId !== userId) {
      throw new BadRequestException('Vous n\'êtes pas le destinataire de ce partage');
    }
    ...
    await this.notificationsService.create({
      ...
      type: NotificationType.SYSTEM_NOTIFICATION,
      title: accept ? 'Partage d\'abonnement accepté' : 'Partage d\'abonnement refusé',
      content: accept 
  ...
  async revokeSharing(sharingId: string, userId: string): Promise<SubscriptionSharing> {
    const sharing = await this.sharingRepository.findOne({
      where: { id: sharingId },
      relations: ['subscription']
    });
    ...
    if (sharing.subscription.userId !== userId) {
      throw new BadRequestException('Vous n\'êtes pas autorisé à révoquer ce partage');
    }
  ...
  async getUsageReport(subscriptionId: string, userId: string): Promise<any> {
    ...
    const daysRemaining = Math.ceil((subscription.endDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
    ...
    return {
      ...
      sharingCount: subscription.sharedWith ? subscription.sharedWith.filter(s => s.status === SharingStatus.ACCEPTED).length : 0
    };
  }
  
  private calculateSubscriptionPrice(baseHourlyPrice: number, plan: SubscriptionPlan): number {
    let basePrice = 0;
  ...
  async checkAccess(userId: string, parkingId: string): Promise<{ hasAccess: boolean; subscription?: Subscription; sharing?: SubscriptionSharing }> {
    const now = new Date();
    ...
    const sharing = await this.sharingRepository.createQueryBuilder('sharing')
      ...
      .andWhere('subscription.status = :subscriptionStatus', { subscriptionStatus: SubscriptionStatus.ACTIVE })
    ...
    if (sharing) {
      ...
      const meetsTimeRestrictions = !sharing.startTime || !sharing.endTime || 
        (currentTime >= sharing.startTime && currentTime <= sharing.endTime);
      
      const meetsDayRestrictions = !sharing.allowedDays || sharing.allowedDays.length === 0 || 
