  ...
  export class SubscriptionsController {
    ...
    findAll(
      ...
      if (userId && userId !== req.user.id && req.user.role !== 'admin') {
        throw new ForbiddenException('Vous n\'êtes pas autorisé à voir les abonnements d\'autres utilisateurs');
      }
    ...
    async findOne(@Param('id') id: string, @Request() req: RequestWithUser) {
      ...
      if (subscription.userId !== req.user.id && req.user.role !== 'admin') {
        ...
        const hasAccess = subscription.sharedWith?.some(sharing => 
          sharing.sharedWithUserId === req.user?.id && 
          sharing.status === 'accepted'
        );
        
        if (!hasAccess) {
          throw new ForbiddenException('Vous n\'êtes pas autorisé à voir cet abonnement');
        }
      }
    ...
    update(
      ...
      return this.subscriptionsService.update(id, req.user.id, updateSubscriptionDto, isAdmin);
    }
    ...
    pauseSubscription(
      ...
      const isAdmin = req.user.role === 'admin';
      return this.subscriptionsService.pauseSubscription(id, req.user.id, pauseSubscriptionDto, isAdmin);
    }
    ...
    resumeSubscription(@Param('id') id: string, @Request() req: RequestWithUser) {
      ...
      const isAdmin = req.user.role === 'admin';
      return this.subscriptionsService.resumeSubscription(id, req.user.id, isAdmin);
    }
    ...
    shareSubscription(
      ...
      return this.subscriptionsService.shareSubscription(id, req.user.id, shareSubscriptionDto);
    }
