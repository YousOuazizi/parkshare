  ...
  export class PaymentsController {
    ...
    async findOne(@Param('id') id: string, @Request() req) {
      ...
      if (payment.userId !== req.user.id && req.user.role !== 'admin') {
        throw new ForbiddenException('Vous n\'êtes pas autorisé à voir ce paiement');
      }
    ...
    async findByBookingId(@Param('bookingId') bookingId: string, @Request() req) {
      ...
      if (payments.length > 0 && payments[0].userId !== req.user.id && req.user.role !== 'admin') {
        throw new ForbiddenException('Vous n\'êtes pas autorisé à voir ces paiements');
      }
