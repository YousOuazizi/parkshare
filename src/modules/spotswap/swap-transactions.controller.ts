...
import { SpotSwapService } from './spotswap.service';
  ...
  export class SwapTransactionsController {
    ...
    async findOne(@Param('id') id: string, @Request() req: any) {
      ...
      if (transaction.listingOwnerId !== req.user.id && transaction.offerOwnerId !== req.user.id) {
        throw new ForbiddenException('Vous n\'êtes pas autorisé à voir cette transaction');
      }
