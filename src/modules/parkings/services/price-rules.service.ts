...
export class PriceRulesService {
  ...
  async create(userId: string, createPriceRuleDto: CreatePriceRuleDto): Promise<PriceRule> {
    ...
    if (!parking) {
      throw new NotFoundException(`Parking avec l'id ${createPriceRuleDto.parkingId} non trouvé`);
    }
    
    if (parking.ownerId !== userId) {
      throw new ForbiddenException('Vous n\'êtes pas autorisé à créer des règles de prix pour ce parking');
    }
  ...
  async update(id: string, userId: string, updatePriceRuleDto: UpdatePriceRuleDto): Promise<PriceRule> {
    ...
    if (!parking) {
      throw new NotFoundException(`Parking avec l'id ${priceRule.parkingId} non trouvé`);
    }
    
    if (parking.ownerId !== userId) {
      throw new ForbiddenException('Vous n\'êtes pas autorisé à modifier cette règle de prix');
    }
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (parking.ownerId !== userId) {
      throw new ForbiddenException('Vous n\'êtes pas autorisé à supprimer cette règle de prix');
    }
      ...
      switch (rule.type) {
        case 'time_based':
          ...
          if (rule.hoursRange) {
            ...
            if (timeStr >= rule.hoursRange.start && timeStr <= rule.hoursRange.end) {
              isApplicable = true;
            }
          }
          break;
        ...
        case 'day_based':
          ...
          if (rule.daysOfWeek && rule.daysOfWeek.includes(startDateTime.getDay())) {
            isApplicable = true;
          }
          break;
